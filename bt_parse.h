...
struct bt_config_s {
    char  chunk_file[BT_FILENAME_LEN];//master chunks
    char  has_chunk_file[BT_FILENAME_LEN];//what this node have chunks
    char  output_file[BT_FILENAME_LEN];
    char  peer_list_file[BT_FILENAME_LEN];//what peers we have
    int   max_conn;
    short identity;
...
void print_peer_list(bt_config_t *config);
#endif /* _BT_PARSE_H_ */
