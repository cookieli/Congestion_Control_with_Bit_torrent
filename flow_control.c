...
void detect_first_loss(void *v){
    ...
    if(win->congestion_state == SS){
        ...
        t->start_time_for_CA = millitime(NULL);
        //win->window_size /= 2;
        ...
        win->window_threshold = MAX(win->window_size/2, 2);
        fprintf(stderr, "now the threshold: %d\n", win->window_threshold);
...
static void slow_start(transfer_t *t){
        t->rtt = 2000;//millitime(NULL) - t->time_stamp;
    flow_window_t *win = &t->sender_window;
...
static void congestion_avoid(transfer_t *t){
    flow_window_t *win = &t->sender_window;
    mytime_t now_rtt = millitime(NULL) - t->start_time_for_CA;
    fprintf(stderr, "now rtt is : %lu\n", now_rtt);
    fprintf(stderr, "transfer rtt is : %lu\n", t->rtt);
    uint32_t new_win_size = win->start_win_size_forCA + now_rtt / t->rtt;
    if(new_win_size > win->window_size){
        win->window_size = new_win_size;
        //win->start_win_size_forCA = new_win_size;
        if(new_win_size > win->window_threshold){
            t->rtt = 4000;
        }
        gragh_wind_size(t);
    }
